...
namespace Collections.Tasks {
    ...
    public class Task {
        ...
        public static IEnumerable<string> Tokenize(TextReader reader)
        {
            ...
            {
                string[] strSplit = str.Split(delimeters, StringSplitOptions.RemoveEmptyEntries);
                foreach (var words in strSplit)
                {
                    yield return words;
                }
            }
        ...
        public static IEnumerable<T> DepthTraversalTree<T>(ITreeNode<T> root)
        {
            ...
            while (stack.Count > 0)
            {
                ITreeNode<T> element = stack.Pop();
                yield return element.Data;
                if (element.Children !=null)
                {
                    foreach(var item in element.Children.Reverse())
                    {
                        stack.Push(item);
                    }
                }
        ...
        public static IEnumerable<T> WidthTraversalTree<T>(ITreeNode<T> root) {
            ...
            while (queue.Count > 0)
            {
                ITreeNode<T> element = queue.Dequeue();
                yield return element.Data;
                if (element.Children !=null)
                {
        ...
        public static IEnumerable<T[]> GenerateAllPermutations<T>(T[] source, int count)
        {
            ...
            if(count == 1)
            {
                foreach (var num in source.Select(t => new T[] { t }))
                {
                    yield return num;
                }
            }   
            while(source.Length >= count)
            {
                var a = GenerateAllPermutations(source.Skip(1).ToArray(), count - 1)
                                                      .SelectMany(t =>source.Take(1),(t1,t2) => new T[] { t2}
                                                      .Concat(t1).ToArray());
